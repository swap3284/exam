...
(function($){
	...
	bValidator = function(mainElement, overrideOptions, instanceName){
		...
		var options = {
			...
			errorMessages: {
				en: {
					...
					'phno':			'Please enter a valid Phone Number in format.'
				}
			}
		},
		...
		validator = {
			...
			phno: function(v){
				return this.regex(v, /^\(?[0-9]{3}\)?[-. ]?[0-9]{3}[-. ]?[0-9]{4}$/i);
			},
			...
			url: function(v){
				...
				return this.regex(v,/^(http|https|ftp):\/\/(([A-Z0-9][A-Z0-9_-]*)(\.[A-Z0-9][A-Z0-9_-]*)+.(com|org|net|dk|at|us|tv|info|uk|co.uk|biz|se)$)(:(\d+))?\/?/i);
